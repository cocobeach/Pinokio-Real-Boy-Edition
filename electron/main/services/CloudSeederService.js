...
class CloudSeederService {
  ...
  async generateTarball(project, workspace, options) {
    ...
    // 5. Generate podman-compose.yml with resource limits (Architect Recommendation)
    const composeYml = this.generatePodmanCompose(project, manifest);
    fs.writeFileSync(path.join(packageDir, 'podman-compose.yml'), composeYml, 'utf-8');
    console.log(`[CloudSeederService] Generated podman-compose.yml with resource limits`);

    // 6. Generate rollback-podman.yml (Architect Recommendation: Rollback Readiness)
    const rollbackYml = await this.generateRollbackCompose(project, manifest);
    fs.writeFileSync(path.join(packageDir, 'rollback-podman.yml'), rollbackYml, 'utf-8');
    console.log(`[CloudSeederService] Generated rollback-podman.yml for sovereign rollback`);

    // 7. Generate healthcheck.sh (Architect Recommendation: Post-Deploy Validation)
    const healthcheckScript = this.generateHealthcheckScript(project, manifest);
    fs.writeFileSync(path.join(packageDir, 'healthcheck.sh'), healthcheckScript, 'utf-8');
    fs.chmodSync(path.join(packageDir, 'healthcheck.sh'), '755');
    console.log(`[CloudSeederService] Generated healthcheck.sh for deployment validation`);

    // 8. Generate SHA256SUMS (Architect Recommendation: Artifact Integrity)
    const checksumContent = await this.generateChecksums(packageDir);
    fs.writeFileSync(path.join(packageDir, 'SHA256SUMS'), checksumContent, 'utf-8');
    console.log(`[CloudSeederService] Generated SHA256SUMS for artifact integrity verification`);
...
`;
  ...
  generateDeploymentManifest(project, options) {
    ...
    // Map services and include GPU requirements
    const services = servicesResult.success ? servicesResult.services.map(service => {
      // Determine if service requires GPU
      const requiresGPU = service.requiresGPU || service.resources?.gpu || false;
    ...
    // Determine resource profile based on GPU requirements
    // This enables Proxmox Controller to make intelligent LXC sizing decisions
    const hasGPU = services.some(s => s.requiresGPU);
    const profile = hasGPU ? '7B' : 'default';
    ...
    return {
      ...
      deployment: {
        ...
        profile,  // Architect Recommendation: Resource profiling for LXC sizing
        timestamp: new Date().toISOString(),
...
`;
  ...
  /**
   * Generate SHA256SUMS for artifact integrity verification
   * @param {string} packageDir - Package directory path
   * @returns {Promise<string>} Checksum content
   */
  async generateChecksums(packageDir) {
    console.log('[CloudSeederService] Calculating SHA256 checksums for critical files...');
    ...
    // Add container tarballs if they exist
    const containersDir = path.join(packageDir, 'containers');
    if (fs.existsSync(containersDir)) {
      const containerFiles = fs.readdirSync(containersDir)
        .filter(f => f.endsWith('.tar'))
        .map(f => `containers/${f}`);
      criticalFiles.push(...containerFiles);
    }

    let checksumContent = `# SHA256 Checksums - Artifact Integrity Verification
# Generated by CloudSeederService (Architect Recommendation: Security Integrity)
...
`;
    ...
    for (const file of criticalFiles) {
      ...
      if (fs.existsSync(filePath)) {
        const checksum = await this.calculateDigest(filePath);
        checksumContent += `${checksum}  ${file}\n`;
        console.log(`[CloudSeederService] ${file}: ${checksum.substring(0, 16)}...`);
      }
    }
  ...
  /**
   * Generate podman-compose.yml with resource limits
   * @param {Object} project - Project object
   * @param {Object} manifest - Deployment manifest
   * @returns {string} YAML content
   */
  generatePodmanCompose(project, manifest) {
    // Resource limits from GPU Bench (Story 11.0)
    const GPU_OPTIMIZED_LIMITS = {
      '7b_model': { cpus: 8, memory: '16g', gpu_memory: '8GB' },
      '13b_model': { cpus: 12, memory: '32g', gpu_memory: '16GB' },
      'default': { cpus: 4, memory: '8g', gpu_memory: '4GB' }
    };
...
`;
    ...
    for (const service of services) {
      const serviceName = service.name.replace(/[^a-z0-9]/gi, '_');
      const limits = service.requiresGPU ? GPU_OPTIMIZED_LIMITS['7b_model'] : GPU_OPTIMIZED_LIMITS['default'];

      compose += `  ${serviceName}:
    image: ${service.image || 'ubuntu:22.04'}
    container_name: ${project.name}-${serviceName}
    deploy:
      resources:
        reservations:
          cpus: '${limits.cpus}'
          memory: ${limits.memory}
${service.requiresGPU ? `          devices:
            - driver: nvidia
              count: 1
              capabilities: [gpu]
` : ''}        limits:
          cpus: '${limits.cpus * 2}'
          memory: ${limits.memory.replace(/g$/, '') * 2}g
    environment:
${Object.entries(service.environment || {}).map(([k, v]) => `      - ${k}=${v}`).join('\n')}
      - PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:512
    ports:
${Object.entries(service.ports || {}).map(([internal, external]) => `      - "${external}:${internal}"`).join('\n')}
...
`;
  ...
  /**
   * Generate rollback-podman.yml with stable git tags
   * @param {Object} project - Project object
   * @param {Object} manifest - Deployment manifest
   * @returns {Promise<string>} YAML content
   */
  async generateRollbackCompose(project, manifest) {
    let compose = `version: '3.8'

# Rollback Compose - Reverts to previous stable git tags
# Generated by CloudSeederService (Architect Recommendation: Rollback Readiness)
...
`;
    ...
    for (const service of services) {
      ...
      // Try to get previous git tag
      let previousTag = 'stable'; // Default fallback
      try {
        const repo = project.repositories?.find(r => r.name === service.repository);
        if (repo && fs.existsSync(repo.path)) {
          // Get second-to-last tag (previous stable)
          const tags = execSync('git tag --sort=-version:refname', { cwd: repo.path, encoding: 'utf-8' })
            .trim()
            .split('\n');
          ...
          if (tags.length >= 2) {
            ...
            previousTag = tags[0]; // Only one tag available
          }
        }
      } catch (error) {
        console.warn(`[CloudSeederService] Could not get git tag for ${service.repository}:`, error.message);
      }

      compose += `  ${serviceName}:
    image: ${service.image || 'ubuntu:22.04'}:${previousTag}
    container_name: ${project.name}-${serviceName}-rollback
    # Rollback to stable tag: ${previousTag}
    environment:
${Object.entries(service.environment || {}).map(([k, v]) => `      - ${k}=${v}`).join('\n')}
    ports:
${Object.entries(service.ports || {}).map(([internal, external]) => `      - "${external}:${internal}"`).join('\n')}
...
# 1. Stop current deployment: podman-compose down
# 2. Deploy rollback: podman-compose -f rollback-podman.yml up -d
# 3. Verify health: ./healthcheck.sh
# 4. If successful, tag as stable: git tag -a rollback-$(date +%Y%m%d) -m "Rollback deployment"
`;
  ...
  /**
   * Generate healthcheck.sh script for post-deploy validation
   * @param {Object} project - Project object
   * @param {Object} manifest - Deployment manifest
   * @returns {string} Bash script content
   */
  generateHealthcheckScript(project, manifest) {
    const services = manifest.services || [];

    // Determine API endpoints to check
    const apiEndpoints = [];
    for (const service of services) {
      if (service.ports) {
        const portEntries = Object.entries(service.ports);
        if (portEntries.length > 0) {
          const [internalPort, externalPort] = portEntries[0];
          apiEndpoints.push({
            name: service.name,
            url: `http://localhost:${externalPort}/health`,
            fallbackUrl: `http://localhost:${externalPort}`
          });
        }
      }
    }
    ...
    return `#!/bin/bash
# Healthcheck Script - Post-Deployment Validation
# Generated by CloudSeederService (Architect Recommendation: Post-Deploy Validation)
# Project: ${project.name}

set -e

echo "========================================="
echo "  Healthcheck: ${project.name}"
echo "========================================="
echo ""
...
CONTAINERS=$(podman ps --filter "name=${project.name}" --format "{{.Names}}" | wc -l)

if [ "$CONTAINERS" -eq 0 ]; then
  echo "❌ FAILED: No containers running for project ${project.name}"
  WARNINGS+=("No containers running")
  FAILED=$((FAILED + 1))
else
  echo "✓ PASSED: $CONTAINERS containers running"
fi
...
# Check ${endpoint.name}
echo -n "  - ${endpoint.name} (${endpoint.url})... "
if curl -f -s -o /dev/null -m 5 "${endpoint.url}" 2>/dev/null; then
  echo "✓ OK"
elif curl -f -s -o /dev/null -m 5 "${endpoint.fallbackUrl}" 2>/dev/null; then
  echo "✓ OK (fallback)"
else
  echo "❌ FAILED"
  WARNINGS+=("${endpoint.name} endpoint unreachable")
  FAILED=$((FAILED + 1))
fi
`).join('')}
...
echo "[3/3] Checking container logs for errors..."
ERROR_COUNT=$(podman logs --since 1m --tail 100 ${project.name}- 2>&1 | grep -i "error\\|fatal\\|exception" | wc -l || echo "0")

if [ "$ERROR_COUNT" -gt 5 ]; then
  echo "⚠ WARNING: $ERROR_COUNT errors found in recent logs (threshold: 5)"
  WARNINGS+=("$ERROR_COUNT errors in recent logs")
  FAILED=$((FAILED + 1))
else
  echo "✓ PASSED: Error count acceptable ($ERROR_COUNT)"
fi
...
if [ "$FAILED" -eq 0 ]; then
  ...
  STATUS="fail"
  echo "❌ HEALTHCHECK FAILED ($FAILED checks failed)"
  EXIT_CODE=1
fi
...
# Architect Recommendation: Structured healthcheck output
...
printf '{"status":"%s","warnings":%s,"timestamp":"%s","project":"%s"}\\n' "$STATUS" "$WARNINGS_JSON" "$TIMESTAMP" "${project.name}"
